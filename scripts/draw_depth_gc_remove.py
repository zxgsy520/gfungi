...
def read_depth(files):
    ...
    for line in open(files, 'r'):
        ...
        yield line[0], float(line[3]),  float(line[4])
...
def draw_gc_depth(files, name, remove_list, minx=0, maxx=100, miny=0, maxy=500):
    ...
    remove_id = []
    for line in open(remove_list, 'r'):
        remove_id.append(line.strip().split()[0])

    for seqid, gc, depth in read_depth(files):
        if seqid not in remove_id:
            gc_list.append(gc)
...
#    axs.scatter(gc, depth, s=20, facecolors='none', edgecolor="#107ab0", alpha=1)
    ...
    plt.savefig('%s.decontamination_gc_depth.png' % name, dpi=700)
    plt.savefig("%s.decontamination_gc_depth.pdf" % name)
...
def gc_depth_help(parser):
    ...
    parser.add_argument('-r', '--remove', metavar='FILE', type=str, required=True,
        help='remove ID list.')
...
contact:  %s <%s>\
    ...
    draw_gc_depth(args.gc_depth, args.name, args.remove)
