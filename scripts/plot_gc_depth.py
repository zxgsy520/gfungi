...
def draw_gc_depth(files, name, minx=0, maxx=100, miny=0, maxy=500):
    ...
    mean_x = round(sum(gc_list)/len(gc_list), 2)
    mean_y = round(sum(depth_list)/len(depth_list), 2)
...
#    axs.set_ylabel('Sequencing Depth(X)')
    axs.set_xlabel("GC %% (Average:%s)" % mean_x)
    axs.set_ylabel("Sequencing Depth (Average:%s X)" % mean_y)
...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
        description='''
name:
    plot_gc_depth.py  Draw a gc depth map

attention:
    plot_gc_depth.py -gcd *.stat_gc_depth.tsv -n A18
